...
#define rgblight_timer_init() do { DDRF |= (1<<1);} while(0)
#define rgblight_timer_enable() do { PORTF &= ~(1<<1);} while(0)
#define rgblight_timer_disable() do { PORTF |= (1<<1);} while(0)

#define rgblight_timer_enabled (~PORTF & (1<<1))
...
extern bool led3_indicator_on;
extern struct cRGB led3_indicator_color;
...
void rgblight_set(void)
{
    ...
    if (led3_indicator_on) led[2] = led3_indicator_color;
    ws2812_setleds(led);
}
